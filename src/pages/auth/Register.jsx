...
export default function Register() {
  ...
  const handlePhotoChange = (e) => {
    ...
    if (file) {
      if (file.size > 10 * 1024 * 1024) {
        setError('Dosya boyutu en fazla 10MB olmalıdır.');
        return;
      }

      const allowedTypes = ['image/jpeg', 'image/png', 'image/jpg', 'image/webp', 'application/pdf'];
      if (!allowedTypes.includes(file.type)) {
        setError('Lütfen geçerli bir dosya seçin (PDF, JPG, PNG, WEBP).');
        return;
      }
      ...
      if (file.type.startsWith('image/')) {
        ...
        // PDF dosyası için önizleme gösterme, sadece dosya adını göster
        setPhotoPreview('pdf');
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('Photo File:', photoFile ? { name: photoFile.name, type: photoFile.type, size: photoFile.size } : 'None');
    ...
    try {
      if (userType === 'student') {
        ...
        console.log('Student Request JSON:', requestData);
        ...
        if (photoFile) {
          formDataToSend.append('studentDocument', photoFile);
          console.log('Student Document attached:', photoFile.name);
        } else {
          console.warn('⚠️ No student document attached!');
        }

        console.log('Sending student registration...');
        await authService.registerStudent(formDataToSend);
      } else {
        const formDataToSend = new FormData();
        const requestData = {
        ...
        console.log('Academician Request JSON:', requestData);
        ...
        if (photoFile) {
          ...
          console.log('ID Card Image attached:', photoFile.name);
        } else {
          console.warn('⚠️ No ID card image attached!');
        }

        console.log('Sending academician registration...');
      ...
      console.error('❌ Registration FAILED in Register.jsx');
        ...
        <div className="form-wrapper">
          ...
          <form onSubmit={handleSubmit} className="premium-form">
            ...
            <div className="photo-upload-area">
              {!photoPreview ? (
                <label className="upload-label">
                  <input
                    type="file"
                    accept={userType === 'student' ? 'image/*,.pdf' : 'image/*'}
                  ...
                  <div className="upload-content">
                    ...
                    <span>{userType === 'student' ? 'Öğrenci Belgesi Yükle (PDF/Resim)' : 'Kimlik Fotoğrafı Yükle'}</span>
                  </div>
                </label>
              ) : (
                <div className="preview-area">
                  {photoPreview === 'pdf' ? (
                    <div className="pdf-preview">
                      ...
                      <span>{photoFile?.name || 'PDF Dosyası'}</span>
                    </div>
                  ) : (
            ...
            <button
              ...
              disabled={loading}
            >
              {loading ? 'İşlem Yapılıyor...' : 'Başvuru Yap'}
            </button>
          </form>
